...
if ($adminContent -match '(?s)(<!-- Tabs -->.*?</div>\s*</div>\s*</div>)') {
    ...
    $agencyContent = $agencyContent -replace '(?s)<div class="row g-4">.*?</div>\s*</div>\s*</div>', ($agencyTabsHtml + "`n            </div>`n        </div>")
...
"@
    ...
    if (-not $agencyContent.Contains('custom-tabs')) {
        $agencyContent = $agencyContent -replace '(?i)</head>', $cssTabs
    }
...
"@
    ...
    Write-Host "No match for tabs in admin-products"
}
